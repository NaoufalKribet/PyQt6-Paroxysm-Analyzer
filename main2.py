...
from PyQt6.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QStackedWidget, QDoubleSpinBox, QRadioButton, QSlider
)
...
class MainWindow(QMainWindow):
    ...
    def update_feature_importance_plot(self, results):
        ...
        fig.update_layout(title="Importance des Caractéristiques", template='plotly_dark')
    ...
    def update_metrics_plot(self, results):
        ...
        fig.update_layout(
            barmode='group',
            title=f"Indicateurs de Performance (Indexes) par Classe<br><sup>Macro F1-Score Global: {macro_f1:.3f}</sup>",
            xaxis_title="Classe",
            yaxis_title="Score",
            yaxis_range=[0, 1],
            template='plotly_dark',
    ...
    def update_probability_plots(self, results):
        ...
        ax1.set_title("Évolution Temporelle des Probabilités Prédites")
        ...
        ax2.set_title("Composition des Probabilités (Vue Empilée)")
    ...
    def _handle_external_prediction_results(self, results_df):
        ...
        if not preds_df.empty:
            fig_plot.add_trace(go.Scatter(x=preds_df['Date'], y=preds_df['VRP'], mode='markers', name='Prédiction "Actif"', marker=dict(size=8, color='orangered', symbol='x')))
    ...
    def update_overview_display(self, df):
        ...
        if 'Ramp' in df.columns:
            ...
            if not high_data.empty:
                fig.add_trace(go.Scatter(x=high_data['Date'], y=high_data['VRP'], mode='markers', name='Événement "High"', marker=dict(color='orangered', size=5, opacity=0.7)))

        fig.update_layout(
            title="Aperçu de la Série Temporelle",
            yaxis_title="VRP [MW]",
            template='plotly_dark'
        )
    ...
    def update_feature_plots(self, current_item, previous_item):
        ...
        fig_hist.update_layout(title=f"Distribution de '{feature_name}'", template='plotly_dark')
        ...
        if self.original_df is not None and 'Ramp' in self.original_df.columns:
            ...
            fig_box.update_layout(title=f"'{feature_name}' en fonction du Label", template='plotly_dark')
    ...
    def update_correlation_plot(self):
        ...
        fig.update_layout(title="Matrice de Corrélation", template='plotly_dark')
    ...
    def update_main_plot(self, y_test, y_pred):
        ...
        if self.val_df_for_plot is not None:
            fig.add_trace(go.Scatter(x=self.val_df_for_plot['Date'], y=self.val_df_for_plot['VRP'], mode='markers', name='Données Validation', marker=dict(size=4, opacity=0.5)))
            ...
            if label != 'Calm':
                ...
                if not preds_subset.empty:
                    fig.add_trace(go.Scatter(
                        ...
                        mode='markers', name=f'Prédit ({label})', 
    ...
    def _display_plotly_fig(self, canvas: QWebEngineView, fig: go.Figure):
        """
        Affiche une figure Plotly dans un QWebEngineView avec un fond sombre correct.
        """
        # Convertit la figure en un simple div HTML, sans la structure de page complète
        raw_html = fig.to_html(full_html=False, include_plotlyjs='cdn')
        
        # Enveloppe ce div dans notre propre template HTML avec le style corrigé
        full_html = f"""
        <html>
            <head>
                <style>
                    body {{ 
                        background-color: #1a1a1a; /* La couleur de fond de votre app */
                        margin: 0; 
                        padding: 0; 
                    }}
                </style>
    ...
    def _clear_plotly_view(self, canvas: QWebEngineView, message: str):
        """
        Affiche un message dans un QWebEngineView avec un fond sombre.
        """
        html = f"""
        <html>
            <head>
